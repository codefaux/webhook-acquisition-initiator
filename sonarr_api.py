...
def search_series(query):
    ...
    response = requests.get(f"{SONARR_URL}/api/v3/series/lookup", params={"term": query}, headers=HEADERS)
    response.raise_for_status()
    return response.json()

def get_episodes(series_id):
    """Retrieve episodes for a given series ID."""
    response = requests.get(f"{SONARR_URL}/api/v3/episode", params={"seriesId": series_id}, headers=HEADERS)
    response.raise_for_status()
    return response.json()
...
def import_downloaded_episode(series_id, season_number, episode_number, file_name, sonarr_folder):
    """
    ...
    if not matching_ep:
        raise ValueError(f"Episode S{season_number:02}E{episode_number:02} not found for '{series_id}'")
